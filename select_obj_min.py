...
# Initialize Camera Intel Realsense
    ...
    while True:
        ...
        for i, coordinates in enumerate(boxes):
            ...
            boximage= cv2.img[results[1]:results[3], results[0]:results[2]]
            mpoint = np.ma.masked_equal(boximage, 0, copy=False).min()
            mdist = depth_frame[mpoint]
            class_index = detected_class_indexes[i]
            class_name = class_catalog[class_index]
            if mdist>0:
                info.append((class_name, mdist, [mpoint[0], mpoint[1]]))
